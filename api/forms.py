...
class AICostEstimateForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'desired_location': forms.TextInput(attrs={
                'class': 'form-control',
            ...
            'house_size_sqft': forms.NumberInput(attrs={
                'class': 'form-control',
                'min': '500',
                'max': '10000',
                'required': True
    ...
    def clean_desired_location(self):
        ...
        if location and len(location.strip()) < 3:
            raise forms.ValidationError("Please enter a more specific location (e.g., 'Austin, Texas, USA')")
    ...
    def clean_number_of_children(self):
        ...
        if children is not None and children > 10:
            raise forms.ValidationError("Please enter a reasonable number of children (max 10)")
        return children
    ...
    def clean_house_size_sqft(self):
        ...
        if size is not None and size > 10000:
            raise forms.ValidationError("House size seems unreasonably large (max 10,000 sq ft)")
        return size
