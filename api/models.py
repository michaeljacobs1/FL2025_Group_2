...
class IncomeEntry(models.Model):
    ...
    costs = models.DecimalField(max_digits=12, decimal_places=2, blank=True, null=True, help_text="Annual costs/expenses")
    location = models.CharField(max_length=200, blank=True, null=True, help_text="Location for this year")
...
class SpendingPreference(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    housing_spending = models.CharField(max_length=20, choices=SPENDING_CHOICES, default="average")
    travel_spending = models.CharField(max_length=20, choices=SPENDING_CHOICES, default="average")
    food_spending = models.CharField(max_length=20, choices=SPENDING_CHOICES, default="average")
    leisure_spending = models.CharField(max_length=20, choices=SPENDING_CHOICES, default="average")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class AICostEstimate(models.Model):
    ...
    # User preferences
    desired_location = models.CharField(max_length=200, help_text="City, State, Country")
    ...
    estimated_home_price = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    estimated_monthly_mortgage = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    estimated_property_tax_annual = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    estimated_insurance_annual = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    estimated_utilities_monthly = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    estimated_maintenance_annual = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    
    # Child-related costs
    estimated_childcare_annual = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    estimated_education_annual = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    estimated_child_healthcare_annual = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    estimated_child_food_annual = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    estimated_child_clothing_annual = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    
    # Additional location-based costs
    estimated_transportation_annual = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    estimated_healthcare_annual = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    estimated_groceries_annual = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    ...
    def total_housing_costs_annual(self):
        ...
        if not all([self.estimated_monthly_mortgage, self.estimated_property_tax_annual, 
                   self.estimated_insurance_annual, self.estimated_utilities_monthly, 
                   self.estimated_maintenance_annual]):
            return None
        return (self.estimated_monthly_mortgage * 12 + 
    ...
    def total_child_costs_annual(self):
        ...
        if not all([self.estimated_childcare_annual, self.estimated_education_annual,
                   self.estimated_child_healthcare_annual, self.estimated_child_food_annual,
                   self.estimated_child_clothing_annual]):
            return None
        return (self.estimated_childcare_annual + 
                self.estimated_education_annual + 
                self.estimated_child_healthcare_annual + 
                self.estimated_child_food_annual + 
                self.estimated_child_clothing_annual) * self.number_of_children
    
    @property
    def total_lifestyle_costs_annual(self):
        """Calculate total annual lifestyle costs"""
        if not all([self.estimated_transportation_annual, self.estimated_healthcare_annual,
                   self.estimated_groceries_annual]):
            return None
        return (self.estimated_transportation_annual + 
